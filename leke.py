...
class Course(object):
    ...
    def load_data(self):
        ...
        self.stu_cid = int(soup.find(id='jStuCid')['value'])
        self.lessons = []
        course_referer = 'https://resource.leke.cn/auth/student/resource/study/studyCourseDetail.htm?courseId={}&userId='.format(str(self.course_id))
        course_headers = {
        ...
        lesson_response = self.request_session.post(course_url, headers=course_headers, data='stuCid={}&curPage=1&pageSize=999'.format(str(self.stu_cid)))
        ...
        for c in lesson_data['datas']['page']['dataList']:
            ...
            for l in c['coursewareList']:
                chapter.append(Lesson(self.request_session, course_headers, user_id, self.stu_cid, l['stuCwid'], l['status'], l['totalDuration'], l['courseName']))
...
class Lesson(object):
    ...
    def begin(self):
        lesson_begin_url = 'https://resource.leke.cn/auth/student/resource/study/studyCourseware.htm?stuCwid={}&stuCid={}'.format(str(self.stu_cwid), str(self.stu_cid))
